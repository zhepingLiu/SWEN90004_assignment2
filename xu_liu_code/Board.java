...
public class Board {
	...
	/**
	 * All jailded agents will be put in this HashMap
	 */
			...
			for (int j = 0; j < Const.board_size; j++) {
				...
				for (int ii = -intVision; ii <= intVision; ii++) {
					for (int jj = -intVision; jj <= intVision; jj++) {
						if (Math.sqrt(ii * ii + jj * jj) < vision && i + ii >= 0
								&& i + ii < Const.board_size && j + jj >= 0
								&& j + jj < Const.board_size) {
							neighbourhood[i][j]
	...
	/**
	 * Put the agent in the jail
	 * @param randomAgent
	 */
	public void putAgentInJail(Agent randomAgent) {
		String key = randomAgent.getCoordinate().getX() + ","
				+ randomAgent.getCoordinate().getY();
	...
	/**
	 * calculate the number of jailed agents.
	 * @return
	 */
	...
	/**
	 * find out whether if there is any agent in jail.
	 * @param i
	 * @param j
	 * @return
	 */
	public boolean isAnyAgentInJailed(int i, int j) {
		...
		if (jailedAgents.get(i + "," + j) != null
				&& jailedAgents.get(i + "," + j).size() > 0) {
			return true;
		}
		return false;
	}
